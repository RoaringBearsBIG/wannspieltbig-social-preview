"""Pure versus-image composition (PIL, no network, no app state).

This is the single canonical composition shared by the og:image (WhatsApp/
Bluesky/Discord previews), the twitter:image variant, and — as a copy in
RoaringBot's core/versus_image.py — the Discord reminder/event-cover images.
Changes here MUST be mirrored to /root/roaringbot/core/versus_image.py.
"""
...
log = logging.getLogger("social-preview.image")
...
def _load_background(game: str) -> Image.Image:
    ...
    if path:
        try:
            ...
            log.warning("image: background load failed for %s, using fallback", game)
...
def _load_game_logo(game: str) -> Image.Image | None:
    ...
    try:
        ...
        log.warning("image: game logo load failed for %s", game)
        return None
    if game == "cs":
